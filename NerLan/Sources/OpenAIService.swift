...
enum OpenAIService {
    ...
    static func segmentTranscript(_ raw: String, model: String, apiKey: String) async throws -> String {
        guard !apiKey.isEmpty else { throw APIError.missingKey }
        let system = """
        你是一個只負責加上句尾標點與斷句的文字編輯器。你會收到一段語音辨識（ASR）產生的逐字稿，通常缺少標點。\
        規則：\
        1. 只能加入句尾的句號（中文用「。」，外語用「.」）並在每句後換行，每句一行。\
        2. 絕對不可更動任何原始內容：不可翻譯、改寫、增刪、調整字詞或更改任何字元；簡繁字體與外語（日文、英文、韓文等）原文都必須原封不動保留。\
        3. 只輸出處理後的逐字稿，每句一行，不要加編號，也不要任何其他說明文字。
        """
        var segments: [String] = []
